...
export class SearchPage {
  ...
  callScriptFunction(refresher) {
    ...
    //// Make the request to App scritpt.
  ...
  confirmPurchase(item) {
    ...
    var stateObj = [];
    this.loc.pushState(stateObj, "BuyPage", "/#/search/" + item.item_code);
    this.zone.run(() => { });
  }
  ...
  public callFilter() {
    ...
    if (this.myInput && this.myInput.trim() != '') {
      this.items = this.items1.filter((i) => {
        return (i.item_name.toLowerCase().indexOf(this.myInput.toLowerCase()) > -1);
      });
